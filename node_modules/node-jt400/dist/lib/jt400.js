...
exports.useInMemoryDb = exports.connect = exports.pool = void 0;
...
function createInstance(connection, insertListFun, inMemory) {
    const mixinConnection = function (obj, newConn) {
        ...
        obj.createWriteStream = function (sql, options) {
            return (0, jdbcwritestream_1.createJdbcWriteStream)(obj.batchUpdate, sql, options && options.bufferSize);
        };
    ...
    const jt400 = mixinConnection({
        ...
        ifs() {
            return (0, ifs_1.ifs)(connection.connection);
        },
        defineProgram(opt) {
            ...
            return function run(params, timeout = 3) {
                return Q.nfcall(pgmFunc, JSON.stringify(params), timeout).then(JSON.parse).catch(handleError({ programName: opt.programName, params, timeout }));
            };
        },
...
function pool(config) {
    const javaCon = jvm
        ...
        .createPoolSync(JSON.stringify((0, defaults_1.defaults)(config || {}, defaultConfig)));
...
function connect(config) {
    ...
    return Q.nfcall(createConnection, JSON.stringify((0, defaults_1.defaults)(config || {}, defaultConfig))).then((javaCon) => {
        return createInstance(createConFrom(javaCon), insertListInOneStatment, false);
    });
}
